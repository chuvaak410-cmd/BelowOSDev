...
// порты
ubyte inb(ushort port) { return __asm!ubyte("inb $1, $0", "={ax},N{dx}", port); }
void outb(ushort port, ubyte val) { __asm("outb %al, %dx", "{al},{dx}", val, port); }
void outw(ushort port, ushort val) { __asm("outw %ax, %dx", "{ax},{dx}", val, port); }
ushort inw(ushort port) { return __asm!ushort("inw %dx, %ax", "={ax},{dx}", port); }
// драйвера
void scroll() {
...
char get_char() {
    ...
    if (code == 0x2A || code == 0x36) { shiftPressed = true; return 0; }
    if (code == 0xAA || code == 0xB6) { shiftPressed = false; return 0; }
...
bool isCmd(const char* bufPtr, string cmd) {
    size_t i = 0;
    while (i < cmd.length) {
        if (bufPtr[i] != cmd[i]) return false;
        i++;
    }
...
void printInt(int n) {
    if (n == 0) { OSprint("0"); return; }
    if (n < 0) { OSprint("-"); n = -n; }
    char[12] buf; int i = 10; buf[11] = '\0';
    while (n > 0 && i >= 0) { 
...
int stringToInt(const char* s) {
    ...
    int i = 0;
    if (s[0] == '-') { sign = -1; i++; }
    for (; s[i] >= '0' && s[i] <= '9'; ++i) res = res * 10 + (s[i] - '0');
    return res * sign;
}
...
bool read_sector(uint lba, ushort* buffer) {
    ...
    while (timeout > 0) {
        ubyte s = inb(0x1F7);
        if (s & 0x08) break;      // DRQ (бит 3) установлен — данные готовы
        if (s & 0x01) return false; // ERR установлен — что-то пошло не так
        timeout--;
    }
...
void sleep(uint iterations) {
    for (uint i = 0; i < iterations * 800000; i++) __asm("", "");
}
...
void BSOD(string err) {
    __asm("cli", "");
    for (int i = 0; i < 2000; i++) vram[i] = 0x1F20;
    curX = 0; curY = 0;
    ...
    OSprint("*** ERROR: ", 0x1F); OSprint(err, 0x1F);
    sleep(30);
    ...
    sleep(30);
    OSprint("TRYINING TO RESTART IN 0x0000008E (0xC0000005, 0xBF801111, 0xF9B4C, 0x00000000), PLEASE SEE TEHNICAL INFO.\n", 0x1F); OSprint(err, 0x1F); OSprint("\n");
    sleep(40);
...
void OSmain() {
    ...
    for (int i = 0; i < 2000; i++) vram[i] = 0x0720;
    curX = 0; curY = 0;
    ...
    while (1) {
        ...
        if (isCmd(buf.ptr, "help")) {
            ...
            if (!found) OSprint("  (empty)\n", 0x08);
        }
        else if (isCmd(buf.ptr, "mkdir") || isCmd(buf.ptr, "nfl")) {
            bool isD = isCmd(buf.ptr, "mkdir");
            OSprint("Name: ");
            char[16] name; memset(name.ptr, 0, 16); OSinput(name.ptr, 16);
            int slot = -1;
            for (int i = 0; i < MAX_FILES; i++) if (!fs[i].exists) { slot = i; break; }
            if (slot != -1) {
                fs[slot].exists = true;
                fs[slot].isDir = isD;
                fs[slot].parentIdx = currentDir;
                for (int j = 0; j < 16; j++) fs[slot].name[j] = name[j];
                memset(fs[slot].content.ptr, 0, MAX_FILE_SIZE);
                sync_to_disk();
                OSprint("Object created successfully.\n", 0x02);
            } else OSprint("Error: Disk full.\n", 0x0C);
        }
            ...
            if (idx != -1 && !fs[idx].isDir) {
                ...
                OSprint("\n---------------\n", 0x07);
            } else OSprint("Error reading file.\n", 0x0C);
        }
        else if (isCmd(buf.ptr, "cd")) {
            OSprint("Target: ");
            char[16] name; memset(name.ptr, 0, 16); OSinput(name.ptr, 16);
            if (strCmp(name.ptr, "..")) {
                if (fs[currentDir].parentIdx != -1) currentDir = fs[currentDir].parentIdx;
            } else {
                int idx = findInDir(name.ptr, currentDir);
                if (idx != -1 && fs[idx].isDir) currentDir = idx;
                else OSprint("Invalid directory.\n", 0x0C);
            }
        }
            ...
            if (idx == -1) {
                for (int i = 1; i < MAX_FILES; i++) {
                    bool match = true;
                    for (int j = 0; j < 16; j++) {
                        if (name[j] != fs[i].name[j]) { match = false; break; }
                        if (name[j] == '\0') break;
                    }
            ...
            if (idx != -1) {
                ...
                fs[idx].exists = false;
                for (int j = 0; j < 16; j++) fs[idx].name[j] = 0;
                for (int j = 0; j < MAX_FILE_SIZE; j++) fs[idx].content[j] = 0;
                fs[idx].parentIdx = -1;
                fs[idx].isDir = false;
            ...
            char op = buf[0];
            OSprint("A: "); size_t a = readInt();
            OSprint("B: "); size_t b = readInt();
            OSprint("Result: ");
            if (op == 'a') printInt(a + b);
            else if (op == 's') printInt(a - b);
            else if (op == 'm') printInt(a * b);
            else if (op == 'd') { if (b != 0) printInt(a / b); else OSprint("ERR:DIV0"); }
            OSprint("\n");
        }
